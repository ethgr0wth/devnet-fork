...
${r}`:r,a=a?`${a}
${c}`:c;let m=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(c,i,!0),this.lexer.state.top=m,t.length===0)break;let f=i.at(-1);if(f?.type==="code")break;if(f?.type==="blockquote"){let b=f,u=b.raw+`
`+t.join(`
...
`}strong({tokens:d}){return`<strong>${this.parser.parseInline(d)}</strong>`}em({tokens:d}){return`<em>${this.parser.parseInline(d)}</em>`}codespan({text:d}){return`<code>${pe(d,!0)}</code>`}br(d){return"<br>"}del({tokens:d}){return`<del>${this.parser.parseInline(d)}</del>`}link({href:d,title:e,tokens:t}){let s=this.parser.parseInline(t),a=ps(d);if(a===null)return s;d=a;let i='<a href="'+d+'"';return e&&(i+=' title="'+pe(e)+'"'),i+=">"+s+"</a>",i}image({href:d,title:e,text:t,tokens:s}){s&&(t=this.parser.parseInline(s,this.parser.textRenderer));let a=ps(d);if(a===null)return pe(t);d=a;let i=`<img src="${d}" alt="${t}"`;return e&&(i+=` title="${pe(e)}"`),i+=">",i}text(d){return"tokens"in d&&d.tokens?this.parser.parseInline(d.tokens):"escaped"in d&&d.escaped?d.text:pe(d.text)}},zt=class{strong({text:d}){return d}em({text:d}){return d}codespan({text:d}){return d}del({text:d}){return d}html({text:d}){return d}text({text:d}){return d}link({text:d}){return""+d}image({text:d}){return""+d}br(){return""}checkbox({raw:d}){return d}},ee=class xt{options;renderer;textRenderer;constructor(e){this.options=e||ve,this.options.renderer=this.options.renderer||new Qe,this.renderer=this.options.renderer,this.renderer.options=this.options,this.renderer.parser=this,this.textRenderer=new zt}static parse(e,t){return new xt(t).parse(e)}static parseInline(e,t){return new xt(t).parseInline(e)}parse(e){let t="";for(let s=0;s<e.length;s++){let a=e[s];if(this.options.extensions?.renderers?.[a.type]){let n=a,l=this.options.extensions.renderers[n.type].call({parser:this},n);if(l!==!1||!["space","hr","heading","code","table","blockquote","list","html","def","paragraph","text"].includes(n.type)){t+=l||"";continue}}let i=a;switch(i.type){case"space":{t+=this.renderer.space(i);break}case"hr":{t+=this.renderer.hr(i);break}case"heading":{t+=this.renderer.heading(i);break}case"code":{t+=this.renderer.code(i);break}case"table":{t+=this.renderer.table(i);break}case"blockquote":{t+=this.renderer.blockquote(i);break}case"list":{t+=this.renderer.list(i);break}case"checkbox":{t+=this.renderer.checkbox(i);break}case"html":{t+=this.renderer.html(i);break}case"def":{t+=this.renderer.def(i);break}case"paragraph":{t+=this.renderer.paragraph(i);break}case"text":{t+=this.renderer.text(i);break}default:{let n='Token with "'+i.type+'" type was not found.';if(this.options.silent)return console.error(n),"";throw new Error(n)}}}return t}parseInline(e,t=this.renderer){let s="";for(let a=0;a<e.length;a++){let i=e[a];if(this.options.extensions?.renderers?.[i.type]){let l=this.options.extensions.renderers[i.type].call({parser:this},i);if(l!==!1||!["escape","html","link","image","strong","em","codespan","br","del","text"].includes(i.type)){s+=l||"";continue}}let n=i;switch(n.type){case"escape":{s+=t.text(n);break}case"html":{s+=t.html(n);break}case"link":{s+=t.link(n);break}case"image":{s+=t.image(n);break}case"checkbox":{s+=t.checkbox(n);break}case"strong":{s+=t.strong(n);break}case"em":{s+=t.em(n);break}case"codespan":{s+=t.codespan(n);break}case"br":{s+=t.br(n);break}case"del":{s+=t.del(n);break}case"text":{s+=t.text(n);break}default:{let l='Token with "'+n.type+'" type was not found.';if(this.options.silent)return console.error(l),"";throw new Error(l)}}}return s}},_e=class{options;block;constructor(d){this.options=d||ve}static passThroughHooks=new Set(["preprocess","postprocess","processAllTokens","emStrongMask"]);static passThroughHooksRespectAsync=new Set(["preprocess","postprocess","processAllTokens"]);preprocess(d){return d}postprocess(d){return d}processAllTokens(d){return d}emStrongMask(d){return d}provideLexer(){return this.block?K.lex:K.lexInline}provideParser(){return this.block?ee.parse:ee.parseInline}},za=class{defaults=yt();options=this.setOptions;parse=this.parseMarkdown(!0);parseInline=this.parseMarkdown(!1);Parser=ee;Renderer=Qe;TextRenderer=zt;Lexer=K;Tokenizer=Ze;Hooks=_e;constructor(...d){this.use(...d)}walkTokens(d,e){let t=[];for(let s of d)switch(t=t.concat(e.call(this,s)),s.type){case"table":{let a=s;for(let i of a.header)t=t.concat(this.walkTokens(i.tokens,e));for(let i of a.rows)for(let n of i)t=t.concat(this.walkTokens(n.tokens,e));break}case"list":{let a=s;t=t.concat(this.walkTokens(a.items,e));break}default:{let a=s;this.defaults.extensions?.childTokens?.[a.type]?this.defaults.extensions.childTokens[a.type].forEach(i=>{let n=a[i].flat(1/0);t=t.concat(this.walkTokens(n,e))}):a.tokens&&(t=t.concat(this.walkTokens(a.tokens,e)))}}return t}use(...d){let e=this.defaults.extensions||{renderers:{},childTokens:{}};return d.forEach(t=>{let s={...t};if(s.async=this.defaults.async||s.async||!1,t.extensions&&(t.extensions.forEach(a=>{if(!a.name)throw new Error("extension name required");if("renderer"in a){let i=e.renderers[a.name];i?e.renderers[a.name]=function(...n){let l=a.renderer.apply(this,n);return l===!1&&(l=i.apply(this,n)),l}:e.renderers[a.name]=a.renderer}if("tokenizer"in a){if(!a.level||a.level!=="block"&&a.level!=="inline")throw new Error("extension level must be 'block' or 'inline'");let i=e[a.level];i?i.unshift(a.tokenizer):e[a.level]=[a.tokenizer],a.start&&(a.level==="block"?e.startBlock?e.startBlock.push(a.start):e.startBlock=[a.start]:a.level==="inline"&&(e.startInline?e.startInline.push(a.start):e.startInline=[a.start]))}"childTokens"in a&&a.childTokens&&(e.childTokens[a.name]=a.childTokens)}),s.extensions=e),t.renderer){let a=this.defaults.renderer||new Qe(this.defaults);for(let i in t.renderer){if(!(i in a))throw new Error(`renderer '${i}' does not exist`);if(["options","parser"].includes(i))continue;let n=i,l=t.renderer[n],o=a[n];a[n]=(...r)=>{let c=l.apply(a,r);return c===!1&&(c=o.apply(a,r)),c||""}}s.renderer=a}if(t.tokenizer){let a=this.defaults.tokenizer||new Ze(this.defaults);for(let i in t.tokenizer){if(!(i in a))throw new Error(`tokenizer '${i}' does not exist`);if(["options","rules","lexer"].includes(i))continue;let n=i,l=t.tokenizer[n],o=a[n];a[n]=(...r)=>{let c=l.apply(a,r);return c===!1&&(c=o.apply(a,r)),c}}s.tokenizer=a}if(t.hooks){let a=this.defaults.hooks||new _e;for(let i in t.hooks){if(!(i in a))throw new Error(`hook '${i}' does not exist`);if(["options","block"].includes(i))continue;let n=i,l=t.hooks[n],o=a[n];_e.passThroughHooks.has(i)?a[n]=r=>{if(this.defaults.async&&_e.passThroughHooksRespectAsync.has(i))return(async()=>{let m=await l.call(a,r);return o.call(a,m)})();let c=l.call(a,r);return o.call(a,c)}:a[n]=(...r)=>{if(this.defaults.async)return(async()=>{let m=await l.apply(a,r);return m===!1&&(m=await o.apply(a,r)),m})();let c=l.apply(a,r);return c===!1&&(c=o.apply(a,r)),c}}s.hooks=a}if(t.walkTokens){let a=this.defaults.walkTokens,i=t.walkTokens;s.walkTokens=function(n){let l=[];return l.push(i.call(this,n)),a&&(l=l.concat(a.call(this,n))),l}}this.defaults={...this.defaults,...s}}),this}setOptions(d){return this.defaults={...this.defaults,...d},this}lexer(d,e){return K.lex(d,e??this.defaults)}parser(d,e){return ee.parse(d,e??this.defaults)}parseMarkdown(d){return(e,t)=>{let s={...t},a={...this.defaults,...s},i=this.onError(!!a.silent,!!a.async);if(this.defaults.async===!0&&s.async===!1)return i(new Error("marked(): The async option was set to true by an extension. Remove async: false from the parse options object to return a Promise."));if(typeof e>"u"||e===null)return i(new Error("marked(): input parameter is undefined or null"));if(typeof e!="string")return i(new Error("marked(): input parameter is of type "+Object.prototype.toString.call(e)+", string expected"));if(a.hooks&&(a.hooks.options=a,a.hooks.block=d),a.async)return(async()=>{let n=a.hooks?await a.hooks.preprocess(e):e,l=await(a.hooks?await a.hooks.provideLexer():d?K.lex:K.lexInline)(n,a),o=a.hooks?await a.hooks.processAllTokens(l):l;a.walkTokens&&await Promise.all(this.walkTokens(o,a.walkTokens));let r=await(a.hooks?await a.hooks.provideParser():d?ee.parse:ee.parseInline)(o,a);return a.hooks?await a.hooks.postprocess(r):r})().catch(i);try{a.hooks&&(e=a.hooks.preprocess(e));let n=(a.hooks?a.hooks.provideLexer():d?K.lex:K.lexInline)(e,a);a.hooks&&(n=a.hooks.processAllTokens(n)),a.walkTokens&&this.walkTokens(n,a.walkTokens);let l=(a.hooks?a.hooks.provideParser():d?ee.parse:ee.parseInline)(n,a);return a.hooks&&(l=a.hooks.postprocess(l)),l}catch(n){return i(n)}}}onError(d,e){return t=>{if(t.message+=`
Please report this to https://github.com/markedjs/marked.`,d){let s="<p>An error occurred:</p><pre>"+pe(t.message+"",!0)+"</pre>";return e?Promise.resolve(s):s}if(e)return Promise.reject(t);throw t}}},be=new za;function _(d,e){return be.parse(d,e)}_.options=_.setOptions=function(d){return be.setOptions(d),_.defaults=be.defaults,hs(_.defaults),_};_.getDefaults=yt;_.defaults=ve;_.use=function(...d){return be.use(...d),_.defaults=be.defaults,hs(_.defaults),_};_.walkTokens=function(d,e){return be.walkTokens(d,e)};_.parseInline=be.parseInline;_.Parser=ee;_.parser=ee.parse;_.Renderer=Qe;_.TextRenderer=zt;_.Lexer=K;_.lexer=K.lex;_.Tokenizer=Ze;_.Hooks=_e;_.parse=_;var Za=_.options,Qa=_.setOptions,Ka=_.use,ei=_.walkTokens,ti=_.parseInline;var si=ee.parse,ai=K.lex;var{entries:Cs,setPrototypeOf:Ss,isFrozen:Ma,getPrototypeOf:Aa,getOwnPropertyDescriptor:Ia}=Object,{freeze:W,seal:Q,create:jt}=Object,{apply:Bt,construct:Dt}=typeof Reflect<"u"&&Reflect;W||(W=function(e){return e});Q||(Q=function(e){return e});Bt||(Bt=function(e,t){for(var s=arguments.length,a=new Array(s>2?s-2:0),i=2;i<s;i++)a[i-2]=arguments[i];return e.apply(t,a)});Dt||(Dt=function(e){for(var t=arguments.length,s=new Array(t>1?t-1:0),a=1;a<t;a++)s[a-1]=arguments[a];return new e(...s)});var tt=X(Array.prototype.forEach),_a=X(Array.prototype.lastIndexOf),Ls=X(Array.prototype.pop),je=X(Array.prototype.push),Ca=X(Array.prototype.splice),at=X(String.prototype.toLowerCase),Mt=X(String.prototype.toString),At=X(String.prototype.match),Be=X(String.prototype.replace),Ha=X(String.prototype.indexOf),ja=X(String.prototype.trim),te=X(Object.prototype.hasOwnProperty),U=X(RegExp.prototype.test),De=Ba(TypeError);function X(d){return function(e){e instanceof RegExp&&(e.lastIndex=0);for(var t=arguments.length,s=new Array(t>1?t-1:0),a=1;a<t;a++)s[a-1]=arguments[a];return Bt(d,e,s)}}function Ba(d){return function(){for(var e=arguments.length,t=new Array(e),s=0;s<e;s++)t[s]=arguments[s];return Dt(d,t)}}function A(d,e){let t=arguments.length>2&&arguments[2]!==void 0?arguments[2]:at;Ss&&Ss(d,null);let s=e.length;for(;s--;){let a=e[s];if(typeof a=="string"){let i=t(a);i!==a&&(Ma(e)||(e[s]=i),a=i)}d[a]=!0}return d}function Da(d){for(let e=0;e<d.length;e++)te(d,e)||(d[e]=null);return d}function ne(d){let e=jt(null);for(let[t,s]of Cs(d))te(d,t)&&(Array.isArray(s)?e[t]=Da(s):s&&typeof s=="object"&&s.constructor===Object?e[t]=ne(s):e[t]=s);return e}function Pe(d,e){for(;d!==null;){let s=Ia(d,e);if(s){if(s.get)return X(s.get);if(typeof s.value=="function")return X(s.value)}d=Aa(d)}function t(){return null}return t}var $s=W(["a","abbr","acronym","address","area","article","aside","audio","b","bdi","bdo","big","blink","blockquote","body","br","button","canvas","caption","center","cite","code","col","colgroup","content","data","datalist","dd","decorator","del","details","dfn","dialog","dir","div","dl","dt","element","em","fieldset","figcaption","figure","font","footer","form","h1","h2","h3","h4","h5","h6","head","header","hgroup","hr","html","i","img","input","ins","kbd","label","legend","li","main","map","mark","marquee","menu","menuitem","meter","nav","nobr","ol","optgroup","option","output","p","picture","pre","progress","q","rp","rt","ruby","s","samp","search","section","select","shadow","slot","small","source","spacer","span","strike","strong","style","sub","summary","sup","table","tbody","td","template","textarea","tfoot","th","thead","time","tr","track","tt","u","ul","var","video","wbr"]),It=W(["svg","a","altglyph","altglyphdef","altglyphitem","animatecolor","animatemotion","animatetransform","circle","clippath","defs","desc","ellipse","enterkeyhint","exportparts","filter","font","g","glyph","glyphref","hkern","image","inputmode","line","lineargradient","marker","mask","metadata","mpath","part","path","pattern","polygon","polyline","radialgradient","rect","stop","style","switch","symbol","text","textpath","title","tref","tspan","view","vkern"]),_t=W(["feBlend","feColorMatrix","feComponentTransfer","feComposite","feConvolveMatrix","feDiffuseLighting","feDisplacementMap","feDistantLight","feDropShadow","feFlood","feFuncA","feFuncB","feFuncG","feFuncR","feGaussianBlur","feImage","feMerge","feMergeNode","feMorphology","feOffset","fePointLight","feSpecularLighting","feSpotLight","feTile","feTurbulence"]),Pa=W(["animate","color-profile","cursor","discard","font-face","font-face-format","font-face-name","font-face-src","font-face-uri","foreignobject","hatch","hatchpath","mesh","meshgradient","meshpatch","meshrow","missing-glyph","script","set","solidcolor","unknown","use"]),Ct=W(["math","menclose","merror","mfenced","mfrac","mglyph","mi","mlabeledtr","mmultiscripts","mn","mo","mover","mpadded","mphantom","mroot","mrow","ms","mspace","msqrt","mstyle","msub","msup","msubsup","mtable","mtd","mtext","mtr","munder","munderover","mprescripts"]),Ra=W(["maction","maligngroup","malignmark","mlongdiv","mscarries","mscarry","msgroup","mstack","msline","msrow","semantics","annotation","annotation-xml","mprescripts","none"]),zs=W(["#text"]),Ms=W(["accept","action","align","alt","autocapitalize","autocomplete","autopictureinpicture","autoplay","background","bgcolor","border","capture","cellpadding","cellspacing","checked","cite","class","clear","color","cols","colspan","controls","controlslist","coords","crossorigin","datetime","decoding","default","dir","disabled","disablepictureinpicture","disableremoteplayback","download","draggable","enctype","enterkeyhint","exportparts","face","for","headers","height","hidden","high","href","hreflang","id","inert","inputmode","integrity","ismap","kind","label","lang","list","loading","loop","low","max","maxlength","media","method","min","minlength","multiple","muted","name","nonce","noshade","novalidate","nowrap","open","optimum","part","pattern","placeholder","playsinline","popover","popovertarget","popovertargetaction","poster","preload","pubdate","radiogroup","readonly","rel","required","rev","reversed","role","rows","rowspan","spellcheck","scope","selected","shape","size","sizes","slot","span","srclang","start","src","srcset","step","style","summary","tabindex","title","translate","type","usemap","valign","value","width","wrap","xmlns","slot"]),Ht=W(["accent-height","accumulate","additive","alignment-baseline","amplitude","ascent","attributename","attributetype","azimuth","basefrequency","baseline-shift","begin","bias","by","class","clip","clippathunits","clip-path","clip-rule","color","color-interpolation","color-interpolation-filters","color-profile","color-rendering","cx","cy","d","dx","dy","diffuseconstant","direction","display","divisor","dur","edgemode","elevation","end","exponent","fill","fill-opacity","fill-rule","filter","filterunits","flood-color","flood-opacity","font-family","font-size","font-size-adjust","font-stretch","font-style","font-variant","font-weight","fx","fy","g1","g2","glyph-name","glyphref","gradientunits","gradienttransform","height","href","id","image-rendering","in","in2","intercept","k","k1","k2","k3","k4","kerning","keypoints","keysplines","keytimes","lang","lengthadjust","letter-spacing","kernelmatrix","kernelunitlength","lighting-color","local","marker-end","marker-mid","marker-start","markerheight","markerunits","markerwidth","maskcontentunits","maskunits","max","mask","mask-type","media","method","mode","min","name","numoctaves","offset","operator","opacity","order","orient","orientation","origin","overflow","paint-order","path","pathlength","patterncontentunits","patterntransform","patternunits","points","preservealpha","preserveaspectratio","primitiveunits","r","rx","ry","radius","refx","refy","repeatcount","repeatdur","restart","result","rotate","scale","seed","shape-rendering","slope","specularconstant","specularexponent","spreadmethod","startoffset","stddeviation","stitchtiles","stop-color","stop-opacity","stroke-dasharray","stroke-dashoffset","stroke-linecap","stroke-linejoin","stroke-miterlimit","stroke-opacity","stroke","stroke-width","style","surfacescale","systemlanguage","tabindex","tablevalues","targetx","targety","transform","transform-origin","text-anchor","text-decoration","text-rendering","textlength","type","u1","u2","unicode","values","viewbox","visibility","version","vert-adv-y","vert-origin-x","vert-origin-y","width","word-spacing","wrap","writing-mode","xchannelselector","ychannelselector","x","x1","x2","xmlns","y","y1","y2","z","zoomandpan"]),As=W(["accent","accentunder","align","bevelled","close","columnsalign","columnlines","columnspan","denomalign","depth","dir","display","displaystyle","encoding","fence","frame","height","href","id","largeop","length","linethickness","lspace","lquote","mathbackground","mathcolor","mathsize","mathvariant","maxsize","minsize","movablelimits","notation","numalign","open","rowalign","rowlines","rowspacing","rowspan","rspace","rquote","scriptlevel","scriptminsize","scriptsizemultiplier","selection","separator","separators","stretchy","subscriptshift","supscriptshift","symmetric","voffset","width","xmlns"]),st=W(["xlink:href","xml:id","xlink:title","xml:space","xmlns:xlink"]),Na=Q(/\{\{[\w\W]*|[\w\W]*\}\}/gm),Oa=Q(/<%[\w\W]*|[\w\W]*%>/gm),Fa=Q(/\$\{[\w\W]*/gm),Ga=Q(/^data-[\-\w.\u00B7-\uFFFF]+$/),qa=Q(/^aria-[\-\w]+$/),Hs=Q(/^(?:(?:(?:f|ht)tps?|mailto|tel|callto|sms|cid|xmpp|matrix):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i),Ua=Q(/^(?:\w+script|data):/i),Wa=Q(/[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205F\u3000]/g),js=Q(/^html$/i),Xa=Q(/^[a-z][.\w]*(-[.\w]+)+$/i),Is=Object.freeze({__proto__:null,ARIA_ATTR:qa,ATTR_WHITESPACE:Wa,CUSTOM_ELEMENT:Xa,DATA_ATTR:Ga,DOCTYPE_NAME:js,ERB_EXPR:Oa,IS_ALLOWED_URI:Hs,IS_SCRIPT_OR_DATA:Ua,MUSTACHE_EXPR:Na,TMPLIT_EXPR:Fa}),Re={element:1,attribute:2,text:3,cdataSection:4,entityReference:5,entityNode:6,progressingInstruction:7,comment:8,document:9,documentType:10,documentFragment:11,notation:12},Ya=function(){return typeof window>"u"?null:window},Va=function(e,t){if(typeof e!="object"||typeof e.createPolicy!="function")return null;let s=null,a="data-tt-policy-suffix";t&&t.hasAttribute(a)&&(s=t.getAttribute(a));let i="dompurify"+(s?"#"+s:"");try{return e.createPolicy(i,{createHTML(n){return n},createScriptURL(n){return n}})}catch{return console.warn("TrustedTypes policy "+i+" could not be created."),null}},_s=function(){return{afterSanitizeAttributes:[],afterSanitizeElements:[],afterSanitizeShadowDOM:[],beforeSanitizeAttributes:[],beforeSanitizeElements:[],beforeSanitizeShadowDOM:[],uponSanitizeAttribute:[],uponSanitizeElement:[],uponSanitizeShadowNode:[]}};function Bs(){let d=arguments.length>0&&arguments[0]!==void 0?arguments[0]:Ya(),e=z=>Bs(z);if(e.version="3.3.1",e.removed=[],!d||!d.document||d.document.nodeType!==Re.document||!d.Element)return e.isSupported=!1,e;let{document:t}=d,s=t,a=s.currentScript,{DocumentFragment:i,HTMLTemplateElement:n,Node:l,Element:o,NodeFilter:r,NamedNodeMap:c=d.NamedNodeMap||d.MozNamedAttrMap,HTMLFormElement:m,DOMParser:f,trustedTypes:b}=d,u=o.prototype,h=Pe(u,"cloneNode"),v=Pe(u,"remove"),y=Pe(u,"nextSibling"),w=Pe(u,"childNodes"),k=Pe(u,"parentNode");if(typeof n=="function"){let z=t.createElement("template");z.content&&z.content.ownerDocument&&(t=z.content.ownerDocument)}let S,L="",{implementation:se,createNodeIterator:oe,createDocumentFragment:Y,getElementsByTagName:re}=t,{importNode:J}=s,M=_s();e.isSupported=typeof Cs=="function"&&typeof k=="function"&&se&&se.createHTMLDocument!==void 0;let{MUSTACHE_EXPR:g,ERB_EXPR:T,TMPLIT_EXPR:$,DATA_ATTR:F,ARIA_ATTR:Z,IS_SCRIPT_OR_DATA:he,ATTR_WHITESPACE:Le,CUSTOM_ELEMENT:Oe}=Is,{IS_ALLOWED_URI:Rt}=Is,R=null,Nt=A({},[...$s,...It,..._t,...Ct,...zs]),N=null,Ot=A({},[...Ms,...Ht,...As,...st]),j=Object.seal(jt(null,{tagNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},allowCustomizedBuiltInElements:{writable:!0,configurable:!1,enumerable:!0,value:!1}})),$e=null,nt=null,xe=Object.seal(jt(null,{tagCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeCheck:{writable:!0,configurable:!1,enumerable:!0,value:null}})),Ft=!0,ot=!0,Gt=!1,qt=!0,ye=!1,Fe=!0,ge=!1,rt=!1,lt=!1,we=!1,Ge=!1,qe=!1,Ut=!0,Wt=!1,Ps="user-content-",dt=!0,ze=!1,ke={},ae=null,ct=A({},["annotation-xml","audio","colgroup","desc","foreignobject","head","iframe","math","mi","mn","mo","ms","mtext","noembed","noframes","noscript","plaintext","script","style","svg","template","thead","title","video","xmp"]),Xt=null,Yt=A({},["audio","video","img","source","image","track"]),pt=null,Vt=A({},["alt","class","for","id","label","name","pattern","placeholder","role","summary","title","value","style","xmlns"]),Ue="http://www.w3.org/1998/Math/MathML",We="http://www.w3.org/2000/svg",le="http://www.w3.org/1999/xhtml",Ee=le,mt=!1,ut=null,Rs=A({},[Ue,We,le],Mt),Xe=A({},["mi","mo","mn","ms","mtext"]),Ye=A({},["annotation-xml"]),Ns=A({},["title","style","font","a","script"]),Me=null,Os=["application/xhtml+xml","text/html"],Fs="text/html",P=null,Te=null,Gs=t.createElement("form"),Jt=function(p){return p instanceof RegExp||p instanceof Function},ht=function(){let p=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};if(!(Te&&Te===p)){if((!p||typeof p!="object")&&(p={}),p=ne(p),Me=Os.indexOf(p.PARSER_MEDIA_TYPE)===-1?Fs:p.PARSER_MEDIA_TYPE,P=Me==="application/xhtml+xml"?Mt:at,R=te(p,"ALLOWED_TAGS")?A({},p.ALLOWED_TAGS,P):Nt,N=te(p,"ALLOWED_ATTR")?A({},p.ALLOWED_ATTR,P):Ot,ut=te(p,"ALLOWED_NAMESPACES")?A({},p.ALLOWED_NAMESPACES,Mt):Rs,pt=te(p,"ADD_URI_SAFE_ATTR")?A(ne(Vt),p.ADD_URI_SAFE_ATTR,P):Vt,Xt=te(p,"ADD_DATA_URI_TAGS")?A(ne(Yt),p.ADD_DATA_URI_TAGS,P):Yt,ae=te(p,"FORBID_CONTENTS")?A({},p.FORBID_CONTENTS,P):ct,$e=te(p,"FORBID_TAGS")?A({},p.FORBID_TAGS,P):ne({}),nt=te(p,"FORBID_ATTR")?A({},p.FORBID_ATTR,P):ne({}),ke=te(p,"USE_PROFILES")?p.USE_PROFILES:!1,Ft=p.ALLOW_ARIA_ATTR!==!1,ot=p.ALLOW_DATA_ATTR!==!1,Gt=p.ALLOW_UNKNOWN_PROTOCOLS||!1,qt=p.ALLOW_SELF_CLOSE_IN_ATTR!==!1,ye=p.SAFE_FOR_TEMPLATES||!1,Fe=p.SAFE_FOR_XML!==!1,ge=p.WHOLE_DOCUMENT||!1,we=p.RETURN_DOM||!1,Ge=p.RETURN_DOM_FRAGMENT||!1,qe=p.RETURN_TRUSTED_TYPE||!1,lt=p.FORCE_BODY||!1,Ut=p.SANITIZE_DOM!==!1,Wt=p.SANITIZE_NAMED_PROPS||!1,dt=p.KEEP_CONTENT!==!1,ze=p.IN_PLACE||!1,Rt=p.ALLOWED_URI_REGEXP||Hs,Ee=p.NAMESPACE||le,Xe=p.MATHML_TEXT_INTEGRATION_POINTS||Xe,Ye=p.HTML_INTEGRATION_POINTS||Ye,j=p.CUSTOM_ELEMENT_HANDLING||{},p.CUSTOM_ELEMENT_HANDLING&&Jt(p.CUSTOM_ELEMENT_HANDLING.tagNameCheck)&&(j.tagNameCheck=p.CUSTOM_ELEMENT_HANDLING.tagNameCheck),p.CUSTOM_ELEMENT_HANDLING&&Jt(p.CUSTOM_ELEMENT_HANDLING.attributeNameCheck)&&(j.attributeNameCheck=p.CUSTOM_ELEMENT_HANDLING.attributeNameCheck),p.CUSTOM_ELEMENT_HANDLING&&typeof p.CUSTOM_ELEMENT_HANDLING.allowCustomizedBuiltInElements=="boolean"&&(j.allowCustomizedBuiltInElements=p.CUSTOM_ELEMENT_HANDLING.allowCustomizedBuiltInElements),ye&&(ot=!1),Ge&&(we=!0),ke&&(R=A({},zs),N=[],ke.html===!0&&(A(R,$s),A(N,Ms)),ke.svg===!0&&(A(R,It),A(N,Ht),A(N,st)),ke.svgFilters===!0&&(A(R,_t),A(N,Ht),A(N,st)),ke.mathMl===!0&&(A(R,Ct),A(N,As),A(N,st))),p.ADD_TAGS&&(typeof p.ADD_TAGS=="function"?xe.tagCheck=p.ADD_TAGS:(R===Nt&&(R=ne(R)),A(R,p.ADD_TAGS,P))),p.ADD_ATTR&&(typeof p.ADD_ATTR=="function"?xe.attributeCheck=p.ADD_ATTR:(N===Ot&&(N=ne(N)),A(N,p.ADD_ATTR,P))),p.ADD_URI_SAFE_ATTR&&A(pt,p.ADD_URI_SAFE_ATTR,P),p.FORBID_CONTENTS&&(ae===ct&&(ae=ne(ae)),A(ae,p.FORBID_CONTENTS,P)),p.ADD_FORBID_CONTENTS&&(ae===ct&&(ae=ne(ae)),A(ae,p.ADD_FORBID_CONTENTS,P)),dt&&(R["#text"]=!0),ge&&A(R,["html","head","body"]),R.table&&(A(R,["tbody"]),delete $e.tbody),p.TRUSTED_TYPES_POLICY){if(typeof p.TRUSTED_TYPES_POLICY.createHTML!="function")throw De('TRUSTED_TYPES_POLICY configuration option must provide a "createHTML" hook.');if(typeof p.TRUSTED_TYPES_POLICY.createScriptURL!="function")throw De('TRUSTED_TYPES_POLICY configuration option must provide a "createScriptURL" hook.');S=p.TRUSTED_TYPES_POLICY,L=S.createHTML("")}else S===void 0&&(S=Va(b,a)),S!==null&&typeof L=="string"&&(L=S.createHTML(""));W&&W(p),Te=p}},Zt=A({},[...It,..._t,...Pa]),Qt=A({},[...Ct,...Ra]),qs=function(p){let x=k(p);(!x||!x.tagName)&&(x={namespaceURI:Ee,tagName:"template"});let E=at(p.tagName),H=at(x.tagName);return ut[p.namespaceURI]?p.namespaceURI===We?x.namespaceURI===le?E==="svg":x.namespaceURI===Ue?E==="svg"&&(H==="annotation-xml"||Xe[H]):!!Zt[E]:p.namespaceURI===Ue?x.namespaceURI===le?E==="math":x.namespaceURI===We?E==="math"&&Ye[H]:!!Qt[E]:p.namespaceURI===le?x.namespaceURI===We&&!Ye[H]||x.namespaceURI===Ue&&!Xe[H]?!1:!Qt[E]&&(Ns[E]||!Zt[E]):!!(Me==="application/xhtml+xml"&&ut[p.namespaceURI]):!1},ie=function(p){je(e.removed,{element:p});try{k(p).removeChild(p)}catch{v(p)}},fe=function(p,x){try{je(e.removed,{attribute:x.getAttributeNode(p),from:x})}catch{je(e.removed,{attribute:null,from:x})}if(x.removeAttribute(p),p==="is")if(we||Ge)try{ie(x)}catch{}else try{x.setAttribute(p,"")}catch{}},Kt=function(p){let x=null,E=null;if(lt)p="<remove></remove>"+p;else{let B=At(p,/^[\r\n\t ]+/);E=B&&B[0]}Me==="application/xhtml+xml"&&Ee===le&&(p='<html xmlns="http://www.w3.org/1999/xhtml"><head></head><body>'+p+"</body></html>");let H=S?S.createHTML(p):p;if(Ee===le)try{x=new f().parseFromString(H,Me)}catch{}if(!x||!x.documentElement){x=se.createDocument(Ee,"template",null);try{x.documentElement.innerHTML=mt?L:H}catch{}}let G=x.body||x.documentElement;return p&&E&&G.insertBefore(t.createTextNode(E),G.childNodes[0]||null),Ee===le?re.call(x,ge?"html":"body")[0]:ge?x.documentElement:G},es=function(p){return oe.call(p.ownerDocument||p,p,r.SHOW_ELEMENT|r.SHOW_COMMENT|r.SHOW_TEXT|r.SHOW_PROCESSING_INSTRUCTION|r.SHOW_CDATA_SECTION,null)},gt=function(p){return p instanceof m&&(typeof p.nodeName!="string"||typeof p.textContent!="string"||typeof p.removeChild!="function"||!(p.attributes instanceof c)||typeof p.removeAttribute!="function"||typeof p.setAttribute!="function"||typeof p.namespaceURI!="string"||typeof p.insertBefore!="function"||typeof p.hasChildNodes!="function")},ts=function(p){return typeof l=="function"&&p instanceof l};function de(z,p,x){tt(z,E=>{E.call(e,p,x,Te)})}let ss=function(p){let x=null;if(de(M.beforeSanitizeElements,p,null),gt(p))return ie(p),!0;let E=P(p.nodeName);if(de(M.uponSanitizeElement,p,{tagName:E,allowedTags:R}),Fe&&p.hasChildNodes()&&!ts(p.firstElementChild)&&U(/<[/\w!]/g,p.innerHTML)&&U(/<[/\w!]/g,p.textContent)||p.nodeType===Re.progressingInstruction||Fe&&p.nodeType===Re.comment&&U(/<[/\w]/g,p.data))return ie(p),!0;if(!(xe.tagCheck instanceof Function&&xe.tagCheck(E))&&(!R[E]||$e[E])){if(!$e[E]&&is(E)&&(j.tagNameCheck instanceof RegExp&&U(j.tagNameCheck,E)||j.tagNameCheck instanceof Function&&j.tagNameCheck(E)))return!1;if(dt&&!ae[E]){let H=k(p)||p.parentNode,G=w(p)||p.childNodes;if(G&&H){let B=G.length;for(let V=B-1;V>=0;--V){let ce=h(G[V],!0);ce.__removalCount=(p.__removalCount||0)+1,H.insertBefore(ce,y(p))}}}return ie(p),!0}return p instanceof o&&!qs(p)||(E==="noscript"||E==="noembed"||E==="noframes")&&U(/<\/no(script|embed|frames)/i,p.innerHTML)?(ie(p),!0):(ye&&p.nodeType===Re.text&&(x=p.textContent,tt([g,T,$],H=>{x=Be(x,H," ")}),p.textContent!==x&&(je(e.removed,{element:p.cloneNode()}),p.textContent=x)),de(M.afterSanitizeElements,p,null),!1)},as=function(p,x,E){if(Ut&&(x==="id"||x==="name")&&(E in t||E in Gs))return!1;if(!(ot&&!nt[x]&&U(F,x))){if(!(Ft&&U(Z,x))){if(!(xe.attributeCheck instanceof Function&&xe.attributeCheck(x,p))){if(!N[x]||nt[x]){if(!(is(p)&&(j.tagNameCheck instanceof RegExp&&U(j.tagNameCheck,p)||j.tagNameCheck instanceof Function&&j.tagNameCheck(p))&&(j.attributeNameCheck instanceof RegExp&&U(j.attributeNameCheck,x)||j.attributeNameCheck instanceof Function&&j.attributeNameCheck(x,p))||x==="is"&&j.allowCustomizedBuiltInElements&&(j.tagNameCheck instanceof RegExp&&U(j.tagNameCheck,E)||j.tagNameCheck instanceof Function&&j.tagNameCheck(E))))return!1}else if(!pt[x]){if(!U(Rt,Be(E,Le,""))){if(!((x==="src"||x==="xlink:href"||x==="href")&&p!=="script"&&Ha(E,"data:")===0&&Xt[p])){if(!(Gt&&!U(he,Be(E,Le,"")))){if(E)return!1}}}}}}}return!0},is=function(p){return p!=="annotation-xml"&&At(p,Oe)},ns=function(p){de(M.beforeSanitizeAttributes,p,null);let{attributes:x}=p;if(!x||gt(p))return;let E={attrName:"",attrValue:"",keepAttr:!0,allowedAttributes:N,forceKeepAttr:void 0},H=x.length;for(;H--;){let G=x[H],{name:B,namespaceURI:V,value:ce}=G,Se=P(B),ft=ce,O=B==="value"?ft:ja(ft);if(E.attrName=Se,E.attrValue=O,E.keepAttr=!0,E.forceKeepAttr=void 0,de(M.uponSanitizeAttribute,p,E),O=E.attrValue,Wt&&(Se==="id"||Se==="name")&&(fe(B,p),O=Ps+O),Fe&&U(/((--!?|])>)|<\/(style|title|textarea)/i,O)){fe(B,p);continue}if(Se==="attributename"&&At(O,"href")){fe(B,p);continue}if(E.forceKeepAttr)continue;if(!E.keepAttr){fe(B,p);continue}if(!qt&&U(/\/>/i,O)){fe(B,p);continue}ye&&tt([g,T,$],rs=>{O=Be(O,rs," ")});let os=P(p.nodeName);if(!as(os,Se,O)){fe(B,p);continue}if(S&&typeof b=="object"&&typeof b.getAttributeType=="function"&&!V)switch(b.getAttributeType(os,Se)){case"TrustedHTML":{O=S.createHTML(O);break}case"TrustedScriptURL":{O=S.createScriptURL(O);break}}if(O!==ft)try{V?p.setAttributeNS(V,B,O):p.setAttribute(B,O),gt(p)?ie(p):Ls(e.removed)}catch{fe(B,p)}}de(M.afterSanitizeAttributes,p,null)},Us=function z(p){let x=null,E=es(p);for(de(M.beforeSanitizeShadowDOM,p,null);x=E.nextNode();)de(M.uponSanitizeShadowNode,x,null),ss(x),ns(x),x.content instanceof i&&z(x.content);de(M.afterSanitizeShadowDOM,p,null)};return e.sanitize=function(z){let p=arguments.length>1&&arguments[1]!==void 0?arguments[1]:{},x=null,E=null,H=null,G=null;if(mt=!z,mt&&(z="<!-->"),typeof z!="string"&&!ts(z))if(typeof z.toString=="function"){if(z=z.toString(),typeof z!="string")throw De("dirty is not a string, aborting")}else throw De("toString is not a function");if(!e.isSupported)return z;if(rt||ht(p),e.removed=[],typeof z=="string"&&(ze=!1),ze){if(z.nodeName){let ce=P(z.nodeName);if(!R[ce]||$e[ce])throw De("root node is forbidden and cannot be sanitized in-place")}}else if(z instanceof l)x=Kt("<!---->"),E=x.ownerDocument.importNode(z,!0),E.nodeType===Re.element&&E.nodeName==="BODY"||E.nodeName==="HTML"?x=E:x.appendChild(E);else{if(!we&&!ye&&!ge&&z.indexOf("<")===-1)return S&&qe?S.createHTML(z):z;if(x=Kt(z),!x)return we?null:qe?L:""}x&&lt&&ie(x.firstChild);let B=es(ze?z:x);for(;H=B.nextNode();)ss(H),ns(H),H.content instanceof i&&Us(H.content);if(ze)return z;if(we){if(Ge)for(G=Y.call(x.ownerDocument);x.firstChild;)G.appendChild(x.firstChild);else G=x;return(N.shadowroot||N.shadowrootmode)&&(G=J.call(s,G,!0)),G}let V=ge?x.outerHTML:x.innerHTML;return ge&&R["!doctype"]&&x.ownerDocument&&x.ownerDocument.doctype&&x.ownerDocument.doctype.name&&U(js,x.ownerDocument.doctype.name)&&(V="<!DOCTYPE "+x.ownerDocument.doctype.name+`>
`+V),ye&&tt([g,T,$],ce=>{V=Be(V,ce," ")}),S&&qe?S.createHTML(V):V},e.setConfig=function(){let z=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};ht(z),rt=!0},e.clearConfig=function(){Te=null,rt=!1},e.isValidAttribute=function(z,p,x){Te||ht({});let E=P(z),H=P(p);return as(E,H,x)},e.addHook=function(z,p){typeof p=="function"&&je(M[z],p)},e.removeHook=function(z,p){if(p!==void 0){let x=_a(M[z],p);return x===-1?void 0:Ca(M[z],x,1)[0]}return Ls(M[z])},e.removeHooks=function(z){M[z]=[]},e.removeAllHooks=function(){M=_s()},e}var Ds=Bs();_.setOptions({breaks:!0,gfm:!0});function ue(d){let e=[],t=d.replace(/```[\s\S]*?```/g,i=>(e.push(i),`%%CODEBLOCK_${e.length-1}%%`)),s=[];t=t.replace(/`[^`]+`/g,i=>(s.push(i),`%%INLINECODE_${s.length-1}%%`)),t=t.replace(/@([a-zA-Z0-9_]+)/g,'<a href="#" class="mention" data-mention="$1">@$1</a>').replace(/#([a-zA-Z0-9_]+)/g,'<a href="#" class="hashtag" data-tag="$1">#$1</a>'),s.forEach((i,n)=>{t=t.replace(`%%INLINECODE_${n}%%`,i)}),e.forEach((i,n)=>{t=t.replace(`%%CODEBLOCK_${n}%%`,i)});let a=_.parse(t);return Ds.sanitize(a,{ADD_ATTR:["data-mention","data-tag"],ADD_TAGS:["img"],ADD_URI_SAFE_ATTR:["src"]})}function C(d,e="success"){let t=document.createElement("div"),a={success:{bg:"rgba(16,185,129,0.15)",border:"#10b981",text:"#34d399",icon:"M5 13l4 4L19 7"},error:{bg:"rgba(16,185,129,0.15)",border:"#059669",text:"#f87171",icon:"M6 18L18 6M6 6l12 12"},info:{bg:"rgba(59,130,246,0.15)",border:"#3b82f6",text:"#60a5fa",icon:"M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"}}[e];t.style.cssText=`
  ...
  `,document.body.appendChild(l);let o=document.getElementById("epic-modal-close"),r=()=>{l.style.animation="epicFadeIn 0.2s ease-out reverse",setTimeout(()=>l.remove(),200)};o?.addEventListener("click",r),l.addEventListener("click",c=>{c.target===l&&r()}),document.addEventListener("keydown",function c(m){m.key==="Escape"&&(r(),document.removeEventListener("keydown",c))})}var me="devnetwork_hash",Ne="devnetwork_device_token",it=[{id:"field",question:"What best describes you?",options:[{label:"Founder / CEO",value:"founder"},{label:"Developer / Engineer",value:"developer"},{label:"Designer / Creative",value:"designer"},{label:"Marketer / Growth",value:"marketer"},{label:"Product Manager",value:"product"},{label:"Freelancer / Agency",value:"freelancer"}]},{id:"experience",question:"Where are you on your journey?",options:[{label:"Just getting started \u2014 exploring ideas",value:"exploring"},{label:"Building my first product",value:"building"},{label:"Shipped something, finding PMF",value:"shipped"},{label:"Scaling \u2014 revenue or team growing",value:"scaling"}]},{id:"skills",question:"What's in your toolkit? (pick all that apply)",multiSelect:!0,options:[{label:"Full-Stack Development",value:"fullstack"},{label:"Frontend / UI",value:"frontend"},{label:"Backend / APIs",value:"backend"},{label:"Mobile Apps",value:"mobile"},{label:"AI / Machine Learning",value:"ai"},{label:"UI/UX Design",value:"design"},{label:"No-Code / Low-Code",value:"nocode"},{label:"Growth & Marketing",value:"growth"},{label:"Sales & Biz Dev",value:"sales"},{label:"Content & Copywriting",value:"content"},{label:"DevOps / Cloud",value:"devops"},{label:"Data & Analytics",value:"data"}]},{id:"focus",question:"What are you here for?",options:[{label:"Find a co-founder or teammates",value:"cofounder"},{label:"Get feedback on what I'm building",value:"feedback"},{label:"Connect with other builders",value:"network"},{label:"Find clients or freelance gigs",value:"clients"}]},{id:"interests",question:"What fires you up? (pick all that apply)",multiSelect:!0,options:[{label:"SaaS & Micro-SaaS",value:"saas"},{label:"Indie Hacking",value:"indiehacking"},{label:"AI Products & Tools",value:"ai-products"},{label:"Developer Tools",value:"devtools"},{label:"E-Commerce / DTC",value:"ecommerce"},{label:"Content Creation",value:"content"},{label:"Open Source",value:"opensource"},{label:"Web3 / Crypto",value:"web3"},{label:"Building in Public",value:"buildinpublic"},{label:"Revenue & Monetization",value:"revenue"},{label:"Fundraising & VCs",value:"fundraising"},{label:"Remote & Async Work",value:"remote"}]},{id:"teamSize",question:"How do you like to build?",options:[{label:"Solo \u2014 I do everything myself",value:"solo"},{label:"Small crew (2-4 people)",value:"small"},{label:"Growing team (5-15)",value:"medium"},{label:"I'm flexible",value:"any"}]}],Ja=[{question:"You have a startup idea at 2am. First thing you do?",options:["Open VS Code and start building a prototype \u{1F4BB}","Write down the value prop and target market \u{1F4DD}","Search if someone already built it \u{1F50D}"],hint:"This reveals how you think...",forFields:["software","design","marketing","data","product","devops"]},{question:"Your MVP is ready but the code is messy. You...",options:["Ship it now, refactor later \u2014 users first \u{1F680}","Clean the codebase before anyone sees it \u{1F9F9}","Get 5 users to test it while I fix things in parallel \u26A1"],hint:"Speed vs. craft...",forFields:["software","design","marketing","data","product","devops"]},{question:"A co-founder disagrees on the product direction. You...",options:["Show them the data and let numbers decide \u{1F4CA}","Prototype both ideas and A/B test with users \u{1F9EA}","Have a deep conversation about the vision and align \u{1F91D}"],hint:"How you resolve conflict matters...",forFields:["software","design","marketing","data","product","devops"]},{question:"You just got your first 100 users. Next move?",options:["Talk to every single user to understand their pain \u{1F3AF}","Build the features they're asking for ASAP \u{1F3D7}\uFE0F","Write a launch thread and start building in public \u{1F4E3}"],hint:"Growth mode activated...",forFields:["software","design","marketing","data","product","devops"]},{question:"What's your unfair advantage as a builder?",options:["I can ship a full product solo \u2014 design to deploy \u{1F6E0}\uFE0F","I understand users better than most engineers do \u{1F9E0}","I can get people excited about anything I'm building \u{1F525}"],hint:"Everyone's got one...",forFields:["software","design","marketing","data","product","devops"]}];var Pt=class{appState;wizardState=null;container;selectedSkills=[];feedSocket=null;posts=[];_notifDebounce=null;_currentView="";activeEcosystem=null;userEcosystems=[];defaultEcosystemId="";constructor(){this.appState={mode:"loading",user:null,hash:null},this.container=document.getElementById("wizard-container")||document.getElementById("main-content"),this.init()}setContent(e,t){t&&(this._currentView=t),this.container.scrollTop=0,this.container.innerHTML=e}async init(){try{let t=await fetch("/api/config");if(t.ok){let s=await t.json();this.defaultEcosystemId=s.default_ecosystem_id||""}}catch{}let e=localStorage.getItem(me);if(e){let t=await this.validateHash(e);if(t.requires_2fa){this.showLogin2FA(e);return}if(t.user){this.appState={mode:"app",user:t.user,hash:e},this.showApp();return}localStorage.removeItem(me)}this.appState.mode="wizard",this.showAuthLanding("login")}async validateHash(e,t){try{let s=localStorage.getItem(Ne),a=await fetch("/api/auth/validate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({hash:e,totp_code:t,device_token:s})});if(a.ok){let i=await a.json();return i.requires_2fa?{user:null,requires_2fa:!0}:{user:i.user}}}catch(s){console.error("Auth validation failed:",s)}return{user:null}}showLogin2FA(e){this.container.innerHTML=`
    ...
    `;let s=this.container.querySelector("video");if(s){let a=document.createElement("source");a.src="/static/devnet-intro.mp4",a.type="video/mp4",s.appendChild(a),s.load()}document.getElementById("auth-tab-login").addEventListener("click",()=>this.showAuthLanding("login")),document.getElementById("auth-tab-signup").addEventListener("click",()=>this.showAuthLanding("signup")),document.getElementById("auth-form").addEventListener("submit",async a=>{a.preventDefault();let i=a.target,n=new FormData(i),l=document.getElementById("auth-submit");l.disabled=!0,l.textContent=t?"Signing in\u2026":"Creating account\u2026";try{let o={email:String(n.get("email")||""),password:String(n.get("password")||"")};t||(o.display_name=String(n.get("display_name")||""));let r=await fetch(t?"/api/auth/login":"/api/auth/signup",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}),c=await r.json();if(c.requires_2fa&&c.pending_token){this.showAuthTwoFactor(c.pending_token);return}if(r.ok&&c.success&&c.session_token){localStorage.setItem(me,c.session_token),this.appState={mode:"app",user:c.user,hash:c.session_token},this.showApp();return}this.showAuthError(c.error||"Something went wrong. Please try again.")}catch{this.showAuthError("Network error. Please try again.")}finally{l.disabled=!1,l.textContent=t?"Sign in":"Create account"}})}showAuthError(e){let t=document.getElementById("auth-error");t&&(t.textContent=e,t.classList.remove("hidden"))}showAuthTwoFactor(e){this.container.innerHTML=`
      ...
      <div class="h-full w-full flex flex-col">
        <div class="panel flex-1">
          ...
          <div class="panel-body">
            ...
            `:t.notifications.map(s=>{let a=s.data?.from_user_name||s.data?.from_user||s.from_user?.displayName||s.last_sender||"Someone",i=s.data?.from_id||s.data?.from_user_id||s.from_user?.id||"",n=s.data?.preview||s.preview||"",l=s.type==="group_message",o=s.group_id||"",r=s.group_name||"Group",c=s.count||1;return`
                ...
                <div class="flex items-start gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="text-xs text-zinc-400 mt-1 flex items-center gap-1">
                      ${l&&c>1?`<span>Latest from ${this.escapeHtml(a)}:</span>`:""}
            ...
            <div class="post-card">
              <div class="flex items-start gap-4">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="text-zinc-300 leading-relaxed prose prose-sm prose-invert">${ue(a.content)}</div>
                </div>
              </div>
            </div>
          `).join("")}
      ...
      <div class="post-card mx-0 sm:mx-0 rounded-none sm:rounded-xl border-x-0 sm:border-x" data-post-id="${s.id}">
        <div class="flex items-start gap-3 sm:gap-4">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="text-zinc-300 leading-relaxed text-sm sm:text-base prose prose-sm prose-invert prose-p:my-1 prose-a:text-emerald-400 prose-code:bg-zinc-800 prose-code:px-1 prose-code:rounded break-words">${ue(s.content)}</div>
            ${s.image_url?`
    ...
    `:"")+this.groupMessages.map(t=>{let s=t.user_id===this.appState.user?.id,a=this.formatTimeLocal(t.created_at||t.timestamp),i=s?this.appState.user?.displayName||"You":t.author?.displayName||"Unknown",n=(t.content||"").substring(0,80),l=this.escapeHtml(t.id||""),o=this.escapeHtml(t.user_id||""),r=this.escapeHtml(i),c=this.escapeHtml(n),m=t.reply_to?(()=>{let f=this.escapeHtml(t.reply_to.author_name||"Unknown"),b=this.escapeHtml((t.reply_to.content_preview||"").substring(0,60));return`
          ...
          <div class="chat-msg-wrapper" data-msg-id="${l}" data-reply-user-id="${o}" data-reply-sender="${r}" data-reply-preview="${c}" style="display: flex; justify-content: flex-end; padding: 2px 0;" title="Double-click to reply">
            <div style="max-width: 70%; position: relative; cursor: default;">
              ...
              <div class="chat-bubble-reply" style="background: rgba(255, 255, 255, 0.95); color: #0f0f0f; padding: 10px 14px; border-radius: 18px 18px 4px 18px; box-shadow: 0 2px 8px rgba(0,0,0,0.15); border: 1px solid rgba(255,255,255,0.3); transition: all 0.15s; position: relative;">
                ${t.content?`<div style="font-size: 14px; line-height: 1.45; word-wrap: break-word; font-weight: 500;">${ue(t.content)}</div>`:""}
          ...
          <div class="chat-msg-wrapper" data-msg-id="${l}" data-reply-user-id="${o}" data-reply-sender="${r}" data-reply-preview="${c}" style="display: flex; align-items: flex-end; gap: 8px; padding: 2px 0;" title="Double-click to reply">
            ...
            <div style="max-width: 70%; cursor: default;">
              ...
              <div class="chat-bubble-reply" style="background: linear-gradient(135deg, #2a2a2e, #1f1f23); color: #fafafa; padding: 10px 14px; border-radius: 18px 18px 18px 4px; border: 1px solid #404045; box-shadow: 0 2px 6px rgba(0,0,0,0.25); transition: all 0.15s; position: relative;">
                ${t.content?`<div style="font-size: 14px; line-height: 1.45; word-wrap: break-word;">${ue(t.content)}</div>`:""}
                ${t.image_url?`<img src="${this.escapeHtml(t.image_url)}" alt="Shared" class="chat-img-open" style="max-width: 100%; max-height: 200px; border-radius: 10px; margin-top: 8px; cursor: pointer; display: block;">`:""}
        ...
        <div class="chat-msg-wrapper" data-msg-id="${n}" data-reply-user-id="${l}" data-reply-sender="${o}" data-reply-preview="${r}" style="display: flex; justify-content: flex-end; padding: 2px 0;" title="Double-click to reply">
          <div style="max-width: 70%; position: relative; cursor: default;">
            ${c}
            <div class="chat-bubble-reply" style="background: rgba(255, 255, 255, 0.95); color: #0f0f0f; padding: 10px 14px; border-radius: 18px 18px 4px 18px; box-shadow: 0 2px 8px rgba(0,0,0,0.15); border: 1px solid rgba(255,255,255,0.3); transition: all 0.15s; position: relative;">
              ${e.content?`<div style="font-size: 14px; line-height: 1.45; word-wrap: break-word; font-weight: 500;">${ue(e.content)}</div>`:""}
      ...
      `;{let m=e.author?.displayName?.charAt(0).toUpperCase()||"?",f=["#f97316","#a855f7","#ec4899","#14b8a6","#facc15","#10b981","#3b82f6","#8b5cf6"],b=e.author?.displayName?.charCodeAt(0)%f.length||0,u=f[b];return`
        <div class="chat-msg-wrapper" data-msg-id="${n}" data-reply-user-id="${l}" data-reply-sender="${o}" data-reply-preview="${r}" style="display: flex; align-items: flex-end; gap: 8px; padding: 2px 0;" title="Double-click to reply">
          ...
          <div style="max-width: 70%; cursor: default;">
            ...
            ${c}
            <div class="chat-bubble-reply" style="background: linear-gradient(135deg, #2a2a2e, #1f1f23); color: #fafafa; padding: 10px 14px; border-radius: 18px 18px 18px 4px; border: 1px solid #404045; box-shadow: 0 2px 6px rgba(0,0,0,0.25); transition: all 0.15s; position: relative;">
              ${e.content?`<div style="font-size: 14px; line-height: 1.45; word-wrap: break-word;">${ue(e.content)}</div>`:""}
              ${e.image_url?`<img src="${this.escapeHtml(e.image_url)}" alt="Shared" class="chat-img-open" style="max-width: 100%; max-height: 200px; border-radius: 10px; margin-top: 8px; cursor: pointer; display: block;">`:""}
